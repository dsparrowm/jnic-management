...
export default function ZoneReportsPage() {
  ...
  async function handleViewReport(reportId: string) {
    ...
    try {
      ...
      const summary = await api.getZoneSummary(token, weekOf);
      setData(summary);
    } catch (err) {
      ...
      <div className="space-y-6">
        ...
        {data && (
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-card px-5 py-4 shadow-sm">
            ...
            <Button
              onClick={() => {
                const waiting = data.branches.filter(
                ...
                const coverage =
                  waiting.length > 0
                    ...
                    : "All in-scope reports are in. Forward to state?";
